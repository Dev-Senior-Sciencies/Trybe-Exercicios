...
const createMenu = (obj) => {
  ...
  const restaurant = {
     ...
     order: (string) => arrayVazio.push(string),
/* refencia:https://github.com/tryber/sd-019-c-project-js-unit-tests/blob/heitor-tessaro-js-unit-tests/src/restaurant.js */
     pay() {
      ...
      this.consumption.forEach((element) => {
        if (menuObj.food[element] !== undefined) {
          total += menuObj.food[element];
        }
        if (menuObj.drink[element] !== undefined) {
          total += menuObj.drink[element];
        }
      });
      return total;
    },
/*      pay: () => {
       for (let index = 0; index < arrayVazio.length; index += 1) {
